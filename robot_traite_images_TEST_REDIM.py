...
def saveImg(processed_img,file_path, savedir,my_quality):
    ...
    final_file_name = tail.rsplit('.', maxsplit=1)[0]+"_compressed_reducx"+str(my_factor_r)+".JPG"
...
#                    imgs = glob.glob("/*.JPG")
#                                        
#                    facteurs_reduc = [1, 2, 4, 10, 20]
#                    print("facteurs de reduction :", facteurs_reduc)
#                    
#                    plt.rcParams.update({'font.size': 16})
#                    fig, axs = plt.subplots(1, 7, figsize = (10, 10))
#                    for i in range(len(imgs)):
#                        axs[i].imshow(imgs[i])
#                        axs[i].set_title(facteurs_reduc[i])
#                    
#                    '''
